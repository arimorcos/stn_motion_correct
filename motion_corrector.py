...
class motion_corrector:
    ...
    """
    default_param_file = '/media/arimorcos/4TB External/stn_conv_net/160104_094224/Epoch_3600_weights.pkl'
    ...
    def correct_file(self, file_to_correct, ref_frame):
        ...
        :return: a numpy array containing the motion corrected file
        """

        # Get number of pages and load tiff (hardcoded currently to every other)
        num_pages = tiff_helpers.get_num_tiff_pages(file_to_correct)
        tiff = tiff_helpers.read_tiff(file_to_correct, pages=range(1, num_pages, 2))
        ...
        num_batches = num_frames / self.__batch_size__
        ...
        for batch_ind in range(num_batches):
            ...
            if batch_frames.shape[2] < self.__batch_size__:
                batch_frames[:, :, stop_ind:self.__batch_size__] = 0
            ...
            to_correct = np.stack((to_correct, ref_imgs), 1)

            # Run through the network
            if batch_ind == 0:
                corrected_file = self.stn.process(to_correct)
            else:
                corrected_file = np.stack((corrected_file, self.stn.process(to_correct)), axis=2)

        # Crop to appropriate size
        corrected_file = corrected_file[:, :, 0:num_frames]
